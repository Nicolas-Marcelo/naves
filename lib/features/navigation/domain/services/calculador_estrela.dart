...
class CalculadorEstrela implements CalculadorRota {
  ...
  Rota? calcular({
    ...
    if (origem == destino) {
      return Rota(pontos: [origem], distancia: 0);
    }

    final abertos = <String>{origem};
    final veioDe = <String, String>{};

    final custos = <String, double>{origem: 0};
    ...
    while (abertos.isNotEmpty) {
      ...
      if (atual == destino) {
        return _montarRota(grafo: grafo, veioDe: veioDe, destino: destino);
      }
      ...
      for (final vizinho in grafo.vizinhos(atual)) {
        ...
        final novoCusto = (custos[atual] ?? double.infinity) + distancia;
  ...
  double _calcularHeuristica(Grafo grafo, String origem, String destino) {
  ...
  Rota _montarRota({
    ...
    for (var i = 0; i < pontos.length - 1; i++) {
      distanciaTotal += grafo.distancia(pontos[i], pontos[i + 1]) ?? 0;
    }

    return Rota(pontos: pontos, distancia: distanciaTotal);
  }
}
